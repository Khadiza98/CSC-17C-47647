...
void destroy(TriMatx *);       //De-allocate memory
...
//Create this function to fill the structure with data
TriMatx *fillMat(int rows){
    ...
    for (int i = 0; i < rows; i++) {
        triMt->col[i] = mrkRand() % 9 + 1;       // Randomly assign column sizes from 1 to 9
        triMt->indx[i] = i;                      // initialize index array
        triMt->data[i] = new int[triMt->col[i]]; // Dynamically Allocate data array for each row
    }
    // Fill data array with random 1 digit values
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < triMt->col[i]; j++) {
            triMt->data[i][j] = mrkRand() % 9 + 1; // Random values [1-9]
        }
    }
    return triMt;
}
