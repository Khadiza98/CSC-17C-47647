...
int **fillAry(int *col,int rows){
    int **array=new int*[rows];       // Dynamically allocate memory for rows 
    for(int i=0;i<rows;i++){        
        array[i]=new int[col[i]];     // Dynamically allocate memory for row with variable columns
    }
    for(int i=0;i<rows;i++){
        for(int j=0;j<col[i];j++){    // Now Fill each row with the correct number of columns
...
void prntAry(int **array, int *col, int rows) {
    for(int i = 0; i < rows; i++) {
        for(int j = 0; j < col[i]; j++) {     // Print each row with the correct number of columns
            cout << array[i][j] << " ";
        }
        cout << endl;
    }
